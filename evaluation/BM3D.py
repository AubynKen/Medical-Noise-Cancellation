# -*- coding: utf-8 -*-
"""
Program to perform Blind Motion 3D (BM3D) denoising.
Author: Pinglei He.
Credit: This program is heavily based on Python implementation of BM3D by Huang Liu:
...
def denoise_BM3D(noisy_img_path: str, save_path: str=None):
    """
    Denoise an image and return the de-noised image in OpenCV image object.
    If save_path is specified, save the output image to save_path.
    """
    cv2.setUseOptimized(True)
    img = cv2.imread(noisy_img_path, cv2.IMREAD_GRAYSCALE)  # read the image using OpenCV in grayscale
    basic_img = BM3D_1st_step(img)  # first step of processing
    final_img = BM3D_2nd_step(basic_img, img)  # output
    # save the output image is save_path is specified
    if save_path:
